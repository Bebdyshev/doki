...
export default function RegisterPage() {
  ...
  const getErrorMessage = (error: string): string => {
    if (error.includes('400') || error.includes('already exists') || error.includes('duplicate')) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 8) {
      setError("For better security, please use at least 8 characters for your password.")
      return
    }
        ...
        <Card className="animate-in slide-in-from-bottom-4 duration-1000 delay-200">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {success && (
                <div className="flex items-center space-x-2 text-sm text-green-600 bg-green-50 p-3 rounded-md border border-green-200 animate-in slide-in-from-top-2 duration-300">
                  ...
                  <span>Account created successfully! Welcome to Doki!</span>
                </div>
              )}
            ...
            <div className="mt-6 text-center text-sm animate-in fade-in duration-1000 delay-500">
              ...
              <Link href="/login" className="text-blue-600 hover:underline transition-all duration-200 hover:text-blue-700">
                Sign in
              </Link>
            </div>
          </CardContent>
